...
def extract_route_candidates(repo_path: str) -> list[dict]:
    ...
    for path in root.rglob("*"):
        ...
        if path.suffix.lower() in {".js", ".jsx", ".ts", ".tsx", ".vue"}:
            ...
            for route in WEB_PATH.findall(content):
                _add_page(candidates, route, anchors, relative)
            ...
            for method, route in SPRING_METHOD.findall(content):
                ...
                _add_api(candidates, full_route, endpoint, relative)
        elif path.suffix.lower() == ".py":
            for method, route in FASTAPI_ROUTE.findall(content):
                endpoint = f"{method.upper()} {route}"
                _add_api(candidates, route, endpoint, relative)
...
async def index_system_routes(request: dict) -> list[dict]:
    ...
    result = extract_route_candidates(parsed.repo_path)
    log.info("系统路由索引已生成", extra={"system_id": parsed.system_id, "candidate_count": len(result)})
    return result
...
def _add_page(candidates: dict[str, KnowledgeCandidate], route: str, anchors: list[str], source: str) -> None:
    candidate = candidates.get(route)
    if candidate is None:
        candidates[route] = KnowledgeCandidate(
            kind="page", title=_route_title(route), anchorTexts=[_route_title(route), route, *anchors],
...
def _add_api(candidates: dict[str, KnowledgeCandidate], route: str, endpoint: str, source: str) -> None:
    candidate = candidates.get(route)
    if candidate is None:
        candidates[route] = KnowledgeCandidate(
            kind="api", title=endpoint, anchorTexts=[_route_title(route), route], routePath=route,
